...
function PageInner(){
  ...
  // observe composer composite via DOM hack (keeps components decoupled)
  ...
  // snapshot WS to journal every 2s
  useEffect(()=>{
    let t:any;
    if(live){
      t = setInterval(()=>{
  ...
  }, [live?.ts]); // change interval when ticking
  ...
  // scope data: live or replay frame
  ...
  return (
    <main style={{ padding: 24, fontFamily: "Inter, system-ui, Arial", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginTop: 16 }}>
        <RiskAdaptor />
        <Executor rNow={scopeData ? scopeData.r : 0.5} knobs={{ slipCap: 10, ttl: 800 }} onReview={setReviewIntent} />
      </div>
